...
class DB:
    ...
    def build_select_query(table: str, rows: List[str], filters: KV) -> Query:
        rows_str = ", ".join(rows) if rows else "*"  # Join the rows into a comma-separated string
        filters_str = " AND ".join([f"{key} = %s" for key, value in filters.items()])
    ...
    def select(self, table: str, rows: List[str], filters: KV) -> List[KV]:
        ...
        """
        query = self.build_select_query(table, rows, filters)
        return self.execute_query(query[0], query[1], True)
    ...
    def build_insert_query(table: str, values: KV) -> Query:
        ...
        """
        filters_VALUES = ", ".join(f"{key}" for key in values.keys())  # Join the rows into a comma-separated string
        filters_KEY = ", ".join(f"%s" for value in values.values())
        query = f"INSERT INTO {table} ({filters_VALUES}) VALUES ({filters_KEY})"
        kk = []
    ...
    def insert(self, table: str, values: KV) -> int:
        ...
        """
        query = self.build_insert_query(table, values)
        return self.execute_query(query, list(values.key()), False)
    ...
    def build_update_query(table: str, values: KV, filters: KV) -> Query:
        ...
        :returns: A query string and any placeholder arguments
        """
        values_key = ", ".join(f"{key} = %s" for key in values.keys())  # Join the rows into a comma-separated string
        filters_str = " AND ".join(f"{key} = %s" for key in filters.keys())
        query = f"UPDATE {table} SET {values_key}"
        if filters_str:
    ...
    def update(self, table: str, values: KV, filters: KV) -> int:
        ...
        :returns: The number of rows affected
        """
        query = self.build_update_query(table, values, filters)
        return self.execute_query(query, list(values.key()), False)
    ...
    def delete(self, table: str, filters: KV) -> int:
        ...
        :returns: The number of rows affected
        """
        query = self.build_delete_query(table, filters)
        return self.execute_query(query, list(filters.key()), False)
